...
// Importation des fichiers utilisés dans la fiche logement 
// comme le titre les avis, ...
...
export default function FicheLogement(props) {
  // Utilisation de useParams pour récupérer les paramètres dans l'URL
  let params = useParams()

  // Constante qui récupère les données dans le fichier json
  const annonces = require('../../annonces.json')

  // Constante qui permet de trouver l'information cherchée 
  // dans le fichier json avec.find()
  // Et params qui permet de trouver cet élément cherché 
  // grâce a son id
  const annonce = annonces.find((x) => x.id === params.id)

  // Si l'élément cherché n'est pas trouve Navigate nous redirige 
  // vers la page d'erreur 404
  if (!annonce) {
    ...
    <div className="body"> 
      ...
      <div className="divContainerElements">
        <div>
          ...
          {/* Nous bouclons les tags avec .map() pour pouvoir 
          faire apparître tous les tags de l'annonce */}
